...
function getUbisoftGameNameFromState(stateFilePath) {
  ...
  try {
    ...
    const list = Array.from(candidates).filter(s => {
      ...
      // Filter out paths, registry keys, system variables, and executables
      if (s.includes('\\') || s.includes('/')) return false
      if (s.includes('$') || s.includes('%')) return false
      if (lower.startsWith('hkey_') || lower.startsWith('hklm') || lower.startsWith('hkcu')) return false
      if (lower.endsWith('.exe') || lower.endsWith('.msi') || lower.endsWith('.dll') || lower.endsWith('.ini')) return false
      if (lower.includes('redist') || lower.includes('vc++') || lower.includes('directx') || lower.includes('dxsetup')) return false
      if (lower.includes('firewall') || lower.includes('punkbuster')) return false
      if (lower.includes('games explorer')) return false
