...
// from SteamGridDB (animated first, then static), then falls back to native
// platform CDNs for Steam/GOG.
export async function runBackgroundCoverDownloader(notifyRenderer) {
  ...
  for (const file of files) {
    try {
      ...
      const { game_id: gameId, name, source } = data
          ...
          if (res.ok) {
            ...
            if (iconUrl) {
              ...
              if (hashMatch) {
                ...
                if (dlResult.success) console.log(`[BG-COVER] Downloaded GOG cover for ${name}`)
              }
            }
          }
        } catch (e) {
